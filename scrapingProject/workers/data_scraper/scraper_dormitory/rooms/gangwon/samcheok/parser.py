...
def post_list_parsing_process(**params):
    target_key_info = {
        'multiple_type' : ['uploaded_time', 'post_url', 'uploader', 'view_count', 'post_thumbnail', 'post_title']
    }
    var, soup, key_list, _ = html_type_default_setting(params, target_key_info)
    post_list = extract_children_tag(soup, 'li', child_tag_attrs={'li1'}, is_child_multiple=True)
    for post in post_list:
        span_thumbnail = extract_children_tag(post, 'span', child_tag_attrs={'class':'f1'})
        if span_thumbnail:
            img_thumbnail = extract_children_tag(span_thumbnail, 'img')
            src = extract_attrs(img_thumbnail, 'src')
