...
def post_list_parsing_process(**params):
    ...
    if not tr_list :
        return
    # 2021-01-06 header [번호, 제목, 작성일, 담당부서, 첨부, 조회]
    for tr in tr_list :
...
def post_content_parsing_process(**params):
    ...
    var, soup, key_list, _ = html_type_default_setting(params, target_key_info)
    info_list = extract_children_tag(soup, 'div', child_tag_attrs={'class' :'bodread_file'}, is_child_multiple=True)
    for info in info_list:
        info_text = extract_text(info)
        if '연락처' in info_text:
            var['contact'] = extract_contact_numbers_from_text(info_text)
            break
