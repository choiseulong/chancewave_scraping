...
def post_content_parsing_process(**params):
    ...
    for candidate in postTextCandidateInfo:
        var[candidate] = search_value_in_json_data_using_path(json_data, f'$..{candidate}', is_data_multiple=False)
        if type(var[candidate]) == type(None):
            continue

        if "</div>" in var[candidate]:
            soupData = change_to_soup(var[candidate])
            childDiv = extract_children_tag(soupData, 'div', child_tag_attrs={}, is_child_multiple=True)
            for div in childDiv :
