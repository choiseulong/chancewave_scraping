...
def post_content_parsing_process(**params):
    ...
    var, soup, key_list, _ = html_type_default_setting(params, target_key_info)
    write_info = extract_children_tag(soup, 'ul', {'class' : 'write_info'}, DataStatus.not_multiple)
    li_list = extract_children_tag(write_info, 'li', DataStatus.empty_attrs, DataStatus.multiple)
    for li in li_list:
        li_text = extract_text(li)
        if '담당자' in li_text:
            var['contact'] = extract_contact_numbers_from_text(li_text)
            break
    cont = extract_children_tag(soup, 'div', {'class' : 'board_con'}, DataStatus.not_multiple)
