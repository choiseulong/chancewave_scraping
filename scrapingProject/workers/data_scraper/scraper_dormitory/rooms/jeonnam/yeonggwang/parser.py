...
def post_list_parsing_process(**params):
    target_key_info = {
        'multiple_type' : ['post_url','uploaded_time', 'view_count', 'post_title', 'uploader']
    }
    var, soup, key_list, _ = html_type_default_setting(params, target_key_info)
    tbody = extract_children_tag(soup, 'tbody')
    tr_list = extract_children_tag(tbody, 'tr', is_child_multiple=True)
    # 2021-12-30 header [번호, 제목, 작성자, 작성일, 첨부, 조회] 
    for tr in tr_list:
...
def post_content_parsing_process(**params):
    ...
    var, soup, key_list, _ = html_type_default_setting(params, target_key_info)
    tmp_content = extract_children_tag(soup, 'div', {'class':'board_view_contents'})
