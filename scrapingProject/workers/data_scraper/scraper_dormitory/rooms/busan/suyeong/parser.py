...
def post_list_parsing_process(**params):
    ...
    if not tr_list :
        return
    # 2021-01-06 header [번호, 제목, 담당부서, 작성일, 조회수]
    for tr in tr_list :
...
def post_content_parsing_process(**params):
    ...
    var, soup, key_list, _ = html_type_default_setting(params, target_key_info)
    info = extract_children_tag(soup, 'dd', child_tag_attrs={'class' : 'basic'})
    li_list = extract_children_tag(info, 'li', is_child_multiple=True)
    for li in li_list:
        li_text = extract_text(li)
        if '☎' in li_text:
            li_text_split = li_text.split('☎')
            var['contact'] = li_text_split[1] if len(li_text_split) > 1 else ''
            break
