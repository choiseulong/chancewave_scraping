...
def post_list_parsing_process(**params):
    ...
    contentsBox = extract_children_tag(soup, 'table', {'class' : 'boardList'}, DataStatus.not_multiple)
    tbody = extract_children_tag(contentsBox, 'tbody', DataStatus.empty_attrs, DataStatus.not_multiple)
    ...
    for tr in tr_list:
        ...
        for td_idx, td in enumerate(td_list):
            ...
            if td_idx == 1 :
                ...
                href = extract_attrs(
                    extract_children_tag(td, 'a', {'href' : True}, DataStatus.not_multiple),
                    'href'
                )
                if 'http' not in href :
...
def post_content_parsing_process(**params):
    ...
    var, soup, key_list, _ = html_type_default_setting(params, target_key_info)
    viewContent = extract_children_tag(soup, 'div', {'class' : 'viewContent'}, DataStatus.not_multiple)
