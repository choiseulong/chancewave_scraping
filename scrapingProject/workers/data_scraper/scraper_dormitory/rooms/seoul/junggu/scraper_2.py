...
'''
    ...
    method : GET
    url : http://www.junggu.seoul.kr/tour/content.do?cmsid=14988&gotoPage={page_count}
    header :
        None
    ...
    def scraping_process(self, channel_code, channel_url, dev, full_channel_code):
        ...
        while True:
            ...
            if self.scraping_target:
                # POST 링크로 외부 페이지 연결되므로, 리스트 내용만 스크래핑
...
def post_list_parsing_process(**params):
    target_key_info = {
        'multiple_type': ['post_url', 'post_thumbnail', 'extra_info', 'post_title', 'start_date', 'end_date']
    }
    ...
    for tmp_post_row in post_row_list:
        ...
        tmp_post_info_area = tmp_post_row.find('div', class_='bbs_txt')
        ...
        var['post_title'].append(clean_text(tmp_post_info_area.find('p').text).strip())

        tmp_info_area_list = tmp_post_info_area.find_all('span')
        tmp_datetime_period_area = tmp_info_area_list[0]
        tmp_location_area = tmp_info_area_list[1]

        tmp_datetime_period_str = clean_text(tmp_datetime_period_area.text).strip()
        period_date_list = tmp_datetime_period_str.split('~')
        var['start_date'].append(convert_datetime_string_to_isoformat_datetime(period_date_list[0].strip()))
        var['end_date'].append(convert_datetime_string_to_isoformat_datetime(period_date_list[1].strip()))
