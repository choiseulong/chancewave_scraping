...
'''
    ...
    method : GET
    url : https://www.sdm.go.kr/health/board/1/list?cp={page_count}
    header :
        None
...
'''
    @post info
    method : GET
    url : 'https://www.sdm.go.kr/health/board/1/{post_id}?sdmBoardSeq={post_id}&mode=view'
    header :
        None
...
def post_list_parsing_process(**params):
    ...
    # 게시물 리스트 테이블 영역
    post_list_table_bs = soup.find('table', class_='table')
    ...
    for tmp_post_row in post_row_list:
        for idx, tmp_td in enumerate(tmp_post_row.find_all('td')):
            ...
            if idx == 1:
                post_id_str = str_grab(tmp_td.find('a').get('onclick'), "goView('", "')")
                tmp_query_param = {
                    'sdmBoardSeq': post_id_str,
                    'mode': 'view'
                }
                query = urlencode(tmp_query_param)
                var['post_url'].append(make_absolute_url(in_url='/health/board/1/' + post_id_str + '?' + query,
...
def post_content_parsing_process(**params):
    ...
    content_info_area = soup.find('table', class_='table2')
    ...
    context_area = context_area.find('div', class_='content')
