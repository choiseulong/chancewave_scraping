...
'''
    ...
    method : GET
    url : https://www.seocho.go.kr/site/seocho/ex/bbs/List.do?pageIndex={pageCount}&cbIdx=57&bcIdx=0&searchCondition=subCont
    header :
        None
...
'''
    @post info
    method : GET
    url : https://www.seocho.go.kr/site/seocho/ex/bbs/View.do?cbIdx=57&bcIdx={postId}
    header :
        None
...
class Scraper(ABCScraper):
    def __init__(self, session):
        ...
        self.channel_name = '서초구'
        self.post_board_name = '공지사항'
        self.channel_main_url = 'https://www.seocho.go.kr/'
...
def post_list_parsing_process(**params):
    ...
    table_column_list = ['글번호', '글제목', '담당부서', '등록일', '조회수']

    # 게시물 리스트 테이블 영역
    post_list_table_bs = soup.find('div', class_='board-list')
...
def post_content_parsing_process(**params):
    ...
    for tmp_row_area in content_info_area.find_all('tr'):
        for tmp_info_title, tmp_info_value in zip(tmp_row_area.find_all('th'), tmp_row_area.find_all('td')):
            ...
            if tmp_info_title_text == '글제목':
                var['post_title'] = tmp_info_value_text
            elif tmp_info_title_text == '담당자이름':
                var['uploader'] = tmp_info_value_text
            elif tmp_info_title_text == '담당자연락처':
                var['contact'] = tmp_info_value_text
            elif tmp_info_title_text == '글내용':
