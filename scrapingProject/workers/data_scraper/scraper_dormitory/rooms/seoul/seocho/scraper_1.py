...
'''
    ...
    method : GET
    url : https://www.seocho.go.kr/site/seocho/ex/bbs/List.do?pageIndex={page_count}&cbIdx=69
    header :
        None
...
'''
    @post info
    method : GET
    url : https://www.seocho.go.kr/site/seocho/ex/bbs/View.do?cbIdx=69&bcIdx={post_id}
    header :
        None
...
def post_list_parsing_process(**params):
    ...
    table_column_list = ['글번호', '글제목', '담당기관', '등록일', '조회수']
...
def post_content_parsing_process(**params):
    ...
    for tmp_row_area in content_info_area.find_all('tr'):
        for tmp_info_title, tmp_info_value in zip(tmp_row_area.find_all('th'), tmp_row_area.find_all('td')):
            ...
            elif tmp_info_title_text == '담당기관':
                if var.get('uploader'):
                    var['uploader'] = tmp_info_value_text + ' ' + var['uploader']
                else:
                    var['uploader'] = tmp_info_value_text

            elif tmp_info_title_text == '담당자 Tel.':
