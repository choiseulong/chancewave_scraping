...
'''
    ...
    method : GET
    url : https://www.jongno.go.kr/health/bbs/selectBoardList.do?bbsId=BBSMSTR_000000000548&menuNo=110521&menuId=1105214&pageIndex={page_count}
    header :
        None
...
'''
    @post info
    method : GET
    url : https://www.jongno.go.kr/health/bbs/selectBoardArticle.do?bbsId=BBSMSTR_000000000548&menuNo=110521&menuId=110521&nttId={postId}
    header :
        None
...
def post_list_parsing_process(**params):
    ...
    for tmp_post_row in post_row_list:
        ...
        for idx, tmp_td in enumerate(tmp_post_row.find_all('li')):
            tmp_column_value = tmp_td.text.strip()

            if idx == 1:
                page_move_function_str = tmp_td.find('a').get('href')
                tmp_post_id = str_grab(page_move_function_str, "viewMove('", "')", from_back=True)

                var['post_url'].append(make_absolute_url(
                    in_url='/health/bbs/selectBoardArticle.do?bbsId=BBSMSTR_000000000548&menuNo=110521&menuId=110521&nttId=' + tmp_post_id,
                    channel_main_url=var['response'].url))

            elif idx == 2:
                date_time_object = datetime.strptime(tmp_column_value, '%Y년%m월%d일')
                var['uploaded_time'].append(date_time_object.isoformat())

            elif idx == 3:
                var['view_count'].append(extract_numbers_in_text(tmp_column_value))
