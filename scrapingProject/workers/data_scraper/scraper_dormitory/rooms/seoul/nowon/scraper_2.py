...
'''
    ...
    method : GET
    url : https://www.nowon.kr/health/user/bbs/BD_selectBbsList.do?q_bbsCode=1001&q_estnColumn1=21&q_currPage={page_count}
    header :
        None
...
'''
    @post info
    method : GET
    url : https://www.nowon.kr/health/user/bbs/BD_selectBbs.do?q_bbsCode=1001&q_bbscttSn={postId}
    header :
        None
...
def post_list_parsing_process(**params):
    ...
    table_column_list = ['번호', '제목', '담당부서', '등록일', '조회', '파일']
    ...
    for tmp_row_area in content_info_area.find_all('tr'):
        for tmp_info_title, tmp_info_value in zip(tmp_row_area.find_all('th'), tmp_row_area.find_all('td')):
            ...
            if tmp_info_title_text == '전화번호':
                ...
                # '보건위생과 공중위생팀 김진영' 형식으로, '보건위생과'는 post_subject에 포함되어 있으므로 제외하고 출력
                var['uploader'] = tmp_info_value_text
                if var['uploader']:
                    uploader_info_list = tmp_info_value_text.split(' ')
                    if len(uploader_info_list) > 1:
                        var['uploader'] = ' '.join([f.strip() for f in uploader_info_list[1:] if f.strip()])
                    else:
                        var['uploader'] = ''
