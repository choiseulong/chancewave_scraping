...
def postContentParsingProcess(**params):
    ...
    var, soup, keyList, _ = html_type_default_setting(params, targetKeyInfo)
    bV_contents = extract_children_tag(soup, 'td', {'class' : 'bV_contents'}, childIsNotMultiple)
    postText = extract_text(bV_contents)
    var['postText'] = postText
    var['contact'] = extract_contact_numbers_from_text(postText)
    var['postImageUrl'] = search_img_list_in_contents(bV_contents, var['channelMainUrl'])
    valueList = [var[key] for key in keyList]
