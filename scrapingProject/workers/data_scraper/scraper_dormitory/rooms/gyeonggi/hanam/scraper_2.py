...
'''
    ...
    method : GET
    url = https://www.hanam.go.kr/www/selectBbsNttList.do?key=6381&bbsNo=1764&searchCtgry=&pageUnit=10&searchCnd=all&searchKrwd=&integrDeptCode=&pageIndex={page_count}
    header :
        None
...
'''
    @post info
    method : GET
    url : https://www.hanam.go.kr/www/selectBbsNttView.do?key=6381&bbsNo=1764&nttNo={post_id}&searchCtgry=&searchCnd=all&searchKrwd=&pageIndex=1&integrDeptCode=
    header :
        None
...
def post_list_parsing_process(**params):
    ...
    table_column_list = ['번호', '제목', '파일', '작성자', '행사 시작일', '조회수']
...
def post_content_parsing_process(**params):
    target_key_info = {
        'single_type': ['post_text', 'start_date', 'end_date'],
    ...
    for tmp_info_row in content_info_row_list:
        ...
        for tmp_column_title_area, tmp_column_value_area in zip(tmp_column_title_area_list, tmp_column_value_area_list):
            ...
            if tmp_column_title_text == '문의전화':
                var['contact'] = tmp_column_value_text
            elif tmp_column_title_text == '행사 시작일':
                var['start_date'] = convert_datetime_string_to_isoformat_datetime(tmp_column_value_text)
            elif tmp_column_title_text == '행사 종료일':
                var['end_date'] = convert_datetime_string_to_isoformat_datetime(tmp_column_value_text)
