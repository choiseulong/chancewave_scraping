                ...
                if tmp_sector_name == '강좌정보':
                    if tmp_info_title_text == '교육기간':
                        ...
                        if len(date_info_str_list) == 2:
                            ...
                            var['start_date2'] = datetime.strptime(date_info_str_list[0], '%Y년 %m월 %d일')
                            var['end_date2'] = datetime.strptime(date_info_str_list[1], '%Y년 %m월 %d일')
                        else:
                            var['start_date2'] = datetime.strptime(date_info_str_list[0], '%Y년 %m월 %d일')
                            var['end_date2'] = datetime.strptime(date_info_str_list[0], '%Y년 %m월 %d일')
                    elif tmp_info_title_text == '접수기간':
                        tmp_date_period_str = tmp_info_value_text
                        date_info_str_list = [f.strip() for f in tmp_date_period_str.split('~')]
                        if len(date_info_str_list) == 2:

                            var['start_date'] = datetime.strptime(date_info_str_list[0], '%Y년 %m월 %d일 %H시')
                            var['end_date'] = datetime.strptime(date_info_str_list[1], '%Y년 %m월 %d일 %H시')
                        else:
                            var['start_date'] = datetime.strptime(date_info_str_list[0], '%Y년 %m월 %d일 %H시')
                            var['end_date'] = datetime.strptime(date_info_str_list[0], '%Y년 %m월 %d일 %H시')
