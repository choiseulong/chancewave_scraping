...
'''
    ...
    method : GET
    url : https://edu.ddc.go.kr/ddcedu/courseList2.do?key=20&pageUnit=10&srcStatus=&srcYear=&srcQuarter=&educational=0&srcTitle=&srcCategory=&pageIndex={page_count}
    header :
        None
...
'''
    @post info
    method : GET
    url : http://lib.ddc.go.kr/ddclib/courseView.do?key=40&course={post_id}
    header :
        None
...
def post_list_parsing_process(**params):
    target_key_info = {
        'multiple_type': ['post_url', 'post_title', 'post_content_target', 'is_going_on']
    }
    ...
    table_column_list = ['기관', '강좌명', '접수기간', '대상', '접수/정원', '접수상태']
    ...
    for tmp_post_row in post_row_list:
        ...
        for idx, tmp_td in enumerate(tmp_post_row.find_all('td')):
            ...
            if idx == 0:
                ...
                if clean_text(tmp_td.text).strip().find('마감') > -1:
...
def post_content_parsing_process(**params):
    target_key_info = {
        'single_type': ['post_text', 'contact', 'linked_post_url', 'start_date', 'end_date', 'start_date2', 'end_date2'],
                ...
                if tmp_sector_name == '강좌정보':
                    if tmp_info_title_text == '교육기간':
                        ...
                        if len(date_info_str_list) == 2:
                            ...
                            var['start_date2'] = datetime.strptime(date_info_str_list[0], '%Y년 %m월 %d일').isoformat()
                            var['end_date2'] = datetime.strptime(date_info_str_list[1], '%Y년 %m월 %d일').isoformat()
                        else:
                            var['start_date2'] = datetime.strptime(date_info_str_list[0], '%Y년 %m월 %d일').isoformat()
                            var['end_date2'] = datetime.strptime(date_info_str_list[0], '%Y년 %m월 %d일').isoformat()
                    elif tmp_info_title_text == '접수기간':
                        tmp_date_period_str = tmp_info_value_text
                        date_info_str_list = [f.strip() for f in tmp_date_period_str.split('~')]
                        if len(date_info_str_list) == 2:

                            var['start_date'] = datetime.strptime(date_info_str_list[0], '%Y년 %m월 %d일 %H시').isoformat()
                            var['end_date'] = datetime.strptime(date_info_str_list[1], '%Y년 %m월 %d일 %H시').isoformat()
                        else:
                            var['start_date'] = datetime.strptime(date_info_str_list[0], '%Y년 %m월 %d일 %H시').isoformat()
                            var['end_date'] = datetime.strptime(date_info_str_list[0], '%Y년 %m월 %d일 %H시').isoformat()
