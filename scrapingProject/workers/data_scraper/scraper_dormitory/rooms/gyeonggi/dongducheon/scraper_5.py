...
'''
    ...
    method : GET
    url : https://www.ddc.go.kr/ddc/selectBbsNttList.do?key=104&bbsNo=24&searchCtgry=&pageUnit=10&searchCnd=all&searchKrwd=&integrDeptCode=&pageIndex={page_count}
    header :
        None
...
'''
    @post info
    method : GET
    url : https://www.ddc.go.kr/ddc/selectBbsNttView.do?key=104&bbsNo=24&nttNo={postId}&searchCtgry=&searchCnd=all&searchKrwd=&pageIndex=4&integrDeptCode=
    header :
        None
...
class Scraper(ABCScraper):
    def __init__(self, session):
        ...
        self.channel_main_url = 'https://www.nyj.go.kr'
...
def post_list_parsing_process(**params):
    ...
    table_column_list = ['글번호', '분류', '제목', '첨부파일', '등록일', '조회수']

    # 게시물 리스트 테이블 영역
    post_list_table_bs = soup.find('div', class_='proc_list').find('table')
    ...
    for tmp_post_row in post_row_list:
        ...
        for idx, tmp_td in enumerate(tmp_post_row.find_all('td')):
            ...
            if idx == 0 and tmp_td.text.strip() == '공지' and var['page_count'] != 1:
                break
            elif idx == 1:
...
def post_content_parsing_process(**params):
    ...
    content_info_area = soup.find('div', class_='proc_read').find('table')

    title_area = content_info_area.find('th', class_='board_title')
    var['post_title'] = clean_text(title_area.text).strip()

    title_area.decompose()
    ...
    for tmp_row_area in content_info_area.find_all('tr'):
        for tmp_info_title, tmp_info_value in zip(tmp_row_area.find_all('th'), tmp_row_area.find_all('td')):
            ...
            if tmp_info_title_text == '담당부서':
                var['contact'] = tmp_info_value.find('strong').nextSibling.text.strip()
                tmp_info_value.find('strong').nextSibling.extract()
                var['uploader'] = tmp_info_value.text.strip()
            elif tmp_info_title_text == '게시 기간':
