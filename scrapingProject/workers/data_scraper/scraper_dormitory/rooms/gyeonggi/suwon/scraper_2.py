...
'''
    ...
    method : GET
    url : https://www.suwon.go.kr/web/board/BD_board.list.do?seq=20220112142728476&bbsCd=1034&pageType=&showSummaryYn=N&delDesc=&q_ctgCd=&q_currPage={page_count}&q_sortName=&q_sortOrder=&q_rowPerPage=10&q_searchKeyType=TITLE___1002&q_searchKey=&q_searchVal=
    header :
        None
...
'''
    @post info
    method : GET
    url : https://www.suwon.go.kr/web/board/BD_board.view.do?seq={post_id}&bbsCd=1034
    header :
        None
...
def post_list_parsing_process(**params):
    ...
    table_column_list = ['번호', '제목', '작성자명',  '등록일', '조회']
    ...
    for tmp_post_row in post_row_list:
        for idx, tmp_td in enumerate(tmp_post_row.find_all('td')):
            ...
            if idx == 0:
                ...
                js_view_str = str_grab(tmp_td.find('a').get('onclick'), 'jsView(', ');')
                page_move_data_list = eval('[' + js_view_str + ']')

                tmp_bbs_cd = page_move_data_list[0]
                tmp_seq = page_move_data_list[1]
