        ...
        for tmp_info_title, tmp_info_value in zip(tmp_row_area.find_all('th'), tmp_row_area.find_all('td')):
            ...
            if tmp_info_title_text == '교육명':
                ...
                if len(date_info_str_list) == 2:
                    ...
                    var['start_date'] = datetime.strptime(date_info_str_list[0], '%Y-%m-%d %H시%M분')
                    var['end_date'] = datetime.strptime(date_info_str_list[1], '%Y-%m-%d %H시%M분')
                else:
                    var['start_date'] = datetime.strptime(date_info_str_list[0], '%Y-%m-%d %H시%M분')
                    var['end_date'] = datetime.strptime(date_info_str_list[0], '%Y-%m-%d %H시%M분')
