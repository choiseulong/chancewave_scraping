...
def post_list_parsing_process(**params):
    ...
    if not tr_list :
        return
    # 2021-01-05 header [번호, 제목, 작성자, 작성일, 조회수, 파일]
    for tr in tr_list :
...
def post_content_parsing_process(**params):
    ...
    var, soup, key_list, _ = html_type_default_setting(params, target_key_info)
    tbody = extract_children_tag(soup, 'tbody')
    title_tr = extract_children_tag(tbody, 'tr')
    var['post_title'] = extract_text(title_tr)
    tmp_contents = extract_children_tag(soup, 'td', child_tag_attrs={'class' : 'con'})
